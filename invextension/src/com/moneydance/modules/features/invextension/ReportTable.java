...
/**
 * An extended and formatted of CoolTable by Kurt Riede
 * added multi sort, filter, rendering of columns, rows, and cells
 * @author Dale Furrow
 */
public class ReportTable extends JScrollPane {
        ...
        class StringTableCellRenderer extends DefaultTableCellRenderer {
            ...
            public Component getTableCellRendererComponent(JTable table,
                ...
                if (value instanceof String) {//set Integers to Right
                    ...
                    renderedLabel.setFont(new Font(renderedLabel.getFont().getName(),
                            Font.PLAIN, renderedLabel.getFont().getSize()));
                }
                return cell;
            }
        }
    ...
    public static void adjustColumnPreferredWidths(JTable table, ColSizeOption option) {
        ...
        for (int col = 0; col < table.getColumnCount(); col++) {
            ...
            for (int row = 0; row < table.getRowCount(); row++) {
                ...
                int upSize = 0;
                //workaround--getPreferredSize insufficient for (at least some) negative numbers, so set width based on
                //one size larger
                if ((value instanceof Integer || value instanceof Double)
                        && ((Double) value < -1.0)) {
                    JLabel comp1 = (JLabel) comp;
                    Font f1 = new Font(comp1.getFont().getName(), comp1.getFont().getStyle(), comp1.getFont().getSize() + 1);
                    comp1.setFont(f1);
                    upSize = comp1.getPreferredSize().width;
                }
                int currentWidth = Math.max(table.getColumnModel().getColumn(col).getWidth(), upSize);
                //set to maximum of all obtained widths
                maxwidth = Math.max(comp.getPreferredSize().width, maxwidth);
                maxwidth = Math.max(currentWidth, maxwidth);
            } // for row
            //following code resizes columns to the maximmum of header and contents
            ...
            switch (option) {
                ...
                case MAXCONTCOLRESIZE:
                    ...
                    Component headerComp =
                            ...
                            headerValue, false, false, -1, col); //changed to -1
    ...
    public void setSortedTableHeader() {
        ...
        for (int i = 0; i < model.getColumnCount(); i++) {
            ...
            if (i < this.frozenColumns) {
                ...
                if (i == firstSort && (firstOrder == SortOrder.ASCENDING
                        || firstOrder == SortOrder.DESCENDING)) {
                    column.setHeaderRenderer(new ArrowHeader(this.lockedTable, 1,
                            firstOrder == SortOrder.DESCENDING ? true : false));
                } else if (i == secondSort && (secondOrder == SortOrder.ASCENDING
                        || secondOrder == SortOrder.DESCENDING)) {
                    column.setHeaderRenderer(new ArrowHeader(this.lockedTable, 2,
                            secondOrder == SortOrder.DESCENDING ? true : false));
                } else if (i == thirdSort && (thirdOrder == SortOrder.ASCENDING
                        || thirdOrder == SortOrder.DESCENDING)) {
                ...
                if (i == firstSort && (firstOrder == SortOrder.ASCENDING
                        || firstOrder == SortOrder.DESCENDING)) {
                    column.setHeaderRenderer(new ArrowHeader(this.scrollTable, 1,
                            firstOrder == SortOrder.DESCENDING ? true : false));
                } else if (i == secondSort && (secondOrder == SortOrder.ASCENDING
                        || secondOrder == SortOrder.DESCENDING)) {
                    column.setHeaderRenderer(new ArrowHeader(this.scrollTable, 2,
                            secondOrder == SortOrder.DESCENDING ? true : false));
                } else if (i == thirdSort && (thirdOrder == SortOrder.ASCENDING
                        || thirdOrder == SortOrder.DESCENDING)) {
            ...
            if (i < this.frozenColumns) {
                JLabel header = (JLabel) column.getHeaderRenderer().
                        getTableCellRendererComponent(lockedTable, new Object(), false, false, -1, viewCol);
                header.repaint();
            } else {
                JLabel header = (JLabel) column.getHeaderRenderer().
                        getTableCellRendererComponent(scrollTable, new Object(), false, false, -1, viewCol);
                header.repaint();
            } //
        }

        ReportTable.adjustColumnPreferredWidths(lockedTable, ColSizeOption.MAXCONTCOLRESIZE);
        ReportTable.adjustColumnPreferredWidths(scrollTable, ColSizeOption.MAXCONTCOLRESIZE);
    ...
    class ArrowHeader extends JLabel implements TableCellRenderer {
        ...
        public Component getTableCellRendererComponent(JTable table, Object value,
            ...
            newLabel.setToolTipText("Sort Priority: " + sortPriority + " Order: "
...
//                System.out.println("j: " + j + " ViewCol: " + viewCol + " Value: " + scrollTable.getColumnName(j - frozenColumns));
                if (j < numCols - 1) {
                    copyIn.append("\t");
                }
            }
