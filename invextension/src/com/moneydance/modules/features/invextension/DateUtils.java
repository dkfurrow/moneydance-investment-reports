...
//        baseCal.setTime(DateUtils.truncate(dateToCheck, Calendar.DATE));
        ...
        if (offlimitDates.contains(dateToCheck) || onWeekend) {//changed this, was baseCal
            return false;
        } else {
            return true;
        }
...
//        int nextDayInt = convertToDateInt(DateUtils.truncate(addDays(startDate, 1), Calendar.DATE));
...
//        int prevDayInt = convertToDateInt(DateUtils.truncate(addDays(startDate, -1), Calendar.DATE));
...
//        offlimitDates.add(calculateFloatingHoliday(2, Calendar.MONDAY, year, Calendar.OCTOBER));
        ...
        //offlimitDates.add(addDays(thanksgiving, 1));// --Friday not NYSE Holiday
...
//        nEasterDay = dEasterSunday.getDate();
        ...
        //return new Date(nYear, nGoodFridayMonth, nGoodFridayDay); // old date format
    ...
    private static Date EasterSunday(int nYear) {
        ...
        //return new Date(nYear, nEasterMonth, nEasterDay); //old format for date
